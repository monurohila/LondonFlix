...
function card(item){
  ...
  return `<article class="card">
    <a href="/watch/${item.id}" class="thumb"><img src="${thumb(item.youtubeId)}" onerror="this.onerror=null;this.src='https://img.youtube.com/vi/${item.youtubeId}/hqdefault.jpg'" alt="${item.title} poster"><span class="badge">${item.type}</span></a>
    <div class="card-body">
      <h3>${item.title}</h3>
...
function renderHome(){
  ...
  $('#heroPosters').innerHTML = featured.map(x=>`<a class="poster" href="/watch/${x.id}"><img src="${thumb(x.youtubeId)}" onerror="this.onerror=null;this.src='https://img.youtube.com/vi/${x.youtubeId}/hqdefault.jpg'" alt="${x.title}"><span>${x.title}</span></a>`).join('');
...
function renderWatch(){
  ...
  <div class="watch-info">
    ...
    <aside class="panel"><h3>More like this</h3><div style="display:grid;gap:12px">${state.items.filter(x=>x.id!==item.id && x.type===item.type).slice(0,4).map(x=>`<a href="/watch/${x.id}" style="display:grid;grid-template-columns:104px 1fr;gap:12px;align-items:center"><img src="${thumb(x.youtubeId)}" onerror="this.onerror=null;this.src='https://img.youtube.com/vi/${x.youtubeId}/hqdefault.jpg'" style="width:104px;border-radius:12px"><span><b>${x.title}</b><br><small style="color:#aaaabc">${x.genre} • ${x.rating}</small></span></a>`).join('')}</div></aside>
  </div>`;
}
function bindGlobal(){
